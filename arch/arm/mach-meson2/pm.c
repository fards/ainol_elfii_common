...
static void meson_pm_suspend(void)
{
    ...
    printk("registers debug 1st:\n");
    printk("    HHI_GCLK_MPEG0: 0x%x\n", READ_CBUS_REG(HHI_GCLK_MPEG0));
    printk("    HHI_GCLK_MPEG1: 0x%x\n", READ_CBUS_REG(HHI_GCLK_MPEG1));
    printk("    HHI_GCLK_MPEG2: 0x%x\n", READ_CBUS_REG(HHI_GCLK_MPEG2));
    printk("    HHI_GCLK_OTHER: 0x%x\n", READ_CBUS_REG(HHI_GCLK_OTHER));
    ...
    WRITE_CBUS_REG(A9_0_IRQ_IN0_INTR_MASK, pdata->power_key);     // enable remote interrupt
...
static int meson_pm_prepare(void)
{
    ...
    WRITE_CBUS_REG(WATCHDOG_TC, 0x0); // Disable Watch dog
    meson_sram_push(meson_sram_suspend, meson_cpu_suspend, meson_cpu_suspend_sz);
    return 0;
}
...
static void enable_clock_gate_exception(void)
{
#define HDMI_CTRL_PORT 0x2008
#define TVFE_APB_ERR_CTRL_MUX1                          ((0x1B00+0x80)<<2)
#define TVFE_APB_ERR_CTRL_MUX2                          ((0x1B00+0x81)<<2)

    WRITE_APB_REG(HDMI_CTRL_PORT,  0x8fff);

    WRITE_APB_REG(TVFE_APB_ERR_CTRL_MUX1, 0x8fff8fff);
    WRITE_APB_REG(TVFE_APB_ERR_CTRL_MUX2, 0x8fff);

    WRITE_CBUS_REG(AIU_AUDAC_CTRL1 ,0x8fff);
}
