...
static void __attribute__((naked)) fiq_isr(void)
{
	...
	int_status = aml_read_reg32(CBUS_REG_ADDR(IRQ_STATUS_REG(AM_IRQ0(0))));
    ...
    for (i = 0; i < MAX_FIQ; i++) {
        if ((fiq_index[i] != 0xff) && (fiq_func[i] != NULL)) {
            if (int_status & (1 << IRQ_BIT(fiq_index[i]))) {
                fiq_func[i]();
				aml_write_reg32(CBUS_REG_ADDR(IRQ_CLR_REG(AM_IRQ0(0))),1 << IRQ_BIT(fiq_index[i]));
...
void request_fiq(unsigned fiq, void (*isr)(void))
{
    ...
    for (i = 0; i < MAX_FIQ; i++) {
        if (fiq_index[i] == 0xff) {
            ...
            if (fiq_share == 0) {
				aml_set_reg32_mask(CBUS_REG_ADDR(IRQ_FIQSEL_REG(fiq)), 1 << IRQ_BIT(fiq));
                //SET_CBUS_REG_MASK(IRQ_FIQSEL_REG(fiq), 1 << IRQ_BIT(fiq));
                dsb();
				aml_set_reg32_mask(CBUS_REG_ADDR(IRQ_MASK_REG(fiq)), 1 << IRQ_BIT(fiq));
				//SET_CBUS_REG_MASK(IRQ_MASK_REG(fiq), 1 << IRQ_BIT(fiq));
                //enable_fiq(fiq);
            }
            break;
        }
...
void free_fiq(unsigned fiq, void (*isr)(void))
{
    ...
    for (i = 0; i < MAX_FIQ; i++) {
        if ((fiq_index[i] == fiq) && (fiq_func[i] == isr)) {
            if (fiq_share == 1) {
				aml_clr_reg32_mask(CBUS_REG_ADDR(IRQ_MASK_REG(fiq)), 1 << IRQ_BIT(fiq));
				//CLEAR_CBUS_REG_MASK(IRQ_MASK_REG(fiq), 1 << IRQ_BIT(fiq));
                //disable_fiq(fiq);
                aml_clr_reg32_mask(CBUS_REG_ADDR(IRQ_FIQSEL_REG(fiq)), IRQ_BIT(fiq));
