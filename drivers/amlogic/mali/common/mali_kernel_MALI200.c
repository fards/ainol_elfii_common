...
//printk("pp irq core_num=%d\n", core->core_number);
	        ...
	        )
	{
		...
		/* no progress detected, killed by the watchdog */
   		...
   		MALI_DEBUG_PRINT(2, ("M200: SW-Timeout Rawstat: 0x%x Tile_addr: 0x%x Status: 0x%x 0x%x.\n", irq_readout ,current_tile_addr ,core_status, core->state) );
/* In this case will the system outside cleanup and reset the core */

		#include <mach/am_regs.h>
       		MALI_DEBUG_PRINT(2, ("Mali_ctl_control = 0x%x.\n", READ_CBUS_REG(HHI_MALI_CLK_CNTL)));
