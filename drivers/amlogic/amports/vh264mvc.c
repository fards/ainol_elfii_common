...
static void recycle_vframe(void)
{
    ...
    if((!list_empty(&recycle_list_head))&&
        ...
        if(mvc_buf->view0_buff_id >= 0){
            WRITE_VREG(BUFFER_RECYCLE, (0<<8)|(mvc_buf->view0_buff_id+1));
            mvc_buf->view0_buff_id = -1;
        }
        else if(mvc_buf->view1_buff_id >= 0){
            WRITE_VREG(BUFFER_RECYCLE, (1<<8)|(mvc_buf->view1_buff_id+1));
            mvc_buf->view1_buff_id = -1;
        }
        if((mvc_buf->view0_buff_id < 0)&&
...
static void H264_DECODE_INIT(void)
{
    ...
    WRITE_MPEG_REG(DOS_SW_RESET0, (1<<7) | (1<<6) | (7<<3));
    WRITE_MPEG_REG(DOS_SW_RESET0, 0);
    ...
    WRITE_MPEG_REG(DOS_SW_RESET0, (1<<9) | (1<<8));
    WRITE_MPEG_REG(DOS_SW_RESET0, 0);
...
#ifdef CONFIG_ARCH_MESON6
    WRITE_MPEG_REG(DOS_SW_RESET0, (1<<11));
    WRITE_MPEG_REG(DOS_SW_RESET0, 0);

    READ_MPEG_REG(DOS_SW_RESET0);
    READ_MPEG_REG(DOS_SW_RESET0);
    READ_MPEG_REG(DOS_SW_RESET0);
